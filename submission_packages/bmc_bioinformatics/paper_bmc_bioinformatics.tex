...
A full artifact checklist, benchmark-statistics note, split summary, and token-level reproducibility note are prepared separately as Additional file 1 so that the main paper can stay focused on the benchmark and method claim.
...
To reduce ambiguity between the benchmark claim and the engineering scaffold, we provide Additional file 1, which lists the exact benchmark artifact names, split statistics, tuned-anchor configuration, and final token-family definition used for \texttt{poly-core v1}.
