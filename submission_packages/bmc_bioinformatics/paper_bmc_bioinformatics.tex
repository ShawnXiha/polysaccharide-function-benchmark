...
% Venue-specific draft for BMC Bioinformatics (Research article).
% Fill author metadata, funding, acknowledgements, and repository URL before submission.
...
\author{[Author names and affiliations to be inserted for submission]}
\date{}
...
\begin{abstract}
\textbf{Background:}
Natural polysaccharides are increasingly studied for antioxidant, immunomodulatory, antitumor, and metabolic regulatory activities, yet machine learning for polysaccharide structure-function prediction remains underdeveloped. Existing carbohydrate machine learning tools are largely glycan-centric, whereas natural polysaccharides are typically described by incomplete but heterogeneous structural evidence such as monosaccharide composition, linkage patterns, branching information, and molecular-weight ranges.

\textbf{Results:}
We build a practical benchmark for multi-label function prediction from public polysaccharide resources and examine which structural signals are useful under current public-data conditions. Starting from public-site DoLPHiN ingestion and a filtered benchmark of 4,121 records with 18 function labels, we reproduce classical, sequence, graph, and additional sparse baselines and establish a tuned logistic baseline as a strong sparse anchor. Under a fixed random split, a compact polysaccharide-specific core representation that encodes molecular-weight buckets, branching availability, and residue-set composition improves macro-F1 from 0.2610 to 0.2678. However, under a stricter DOI-grouped split, the tuned logistic anchor remains stronger, and paired bootstrap analysis shows that the random-split gain is not statistically decisive. Controlled ablations identify molecular-weight and residue features as the dominant contributors, with branching providing a smaller positive effect.

\textbf{Conclusions:}
The most reliable contribution of this study is a reproducible benchmark, a strong sparse anchor, and a feature-level understanding of which structural signals are currently useful for natural polysaccharide prediction. The final structured augmentation is informative but split-sensitive rather than robustly general.
\end{abstract}

\noindent\textbf{Keywords:} polysaccharides; bioinformatics; machine learning; benchmark; multi-label classification; weak supervision
...
For the main benchmark split, we use a precomputed fixed partition with 2,472 training records, 824 validation records, and 825 test records. All results reported in the main paper use the test partition only. The validation partition is kept for protocol completeness and future model-selection work, but our Stage 2--4 comparisons intentionally reuse the same fixed train/test view so that later-stage improvements can be attributed to controlled representation or configuration changes rather than split regeneration.
...
\begin{itemize}[leftmargin=*,nosep]
\item \textbf{The benchmark is nontrivial.} The majority baseline reaches only 0.0397 macro-F1, while untuned logistic regression improves to 0.1580. This shows that the label space is not solved by prior frequency alone, but that even a sparse textual view of structure already contains predictive signal.
...
\caption{Main comparison on the \texttt{publishable\_supervised\_v1} benchmark. The strongest untuned baseline is the sequence n-gram model, the strongest tuned anchor is logistic regression, and the final \texttt{poly-core v1} method gives the best macro-F1 under the fixed random-split protocol.}
\label{fig:main_results}
\end{figure}

\subsection{Grouped-Split and Bootstrap Reassessment}

To address the risk that a fixed random split overestimates generalization, we added a deterministic DOI-grouped split in which all records sharing the same DOI are kept in the same partition. This split removes a major source of near-duplicate leakage through paper-specific phrasing and reporting patterns. We also evaluate the tuned logistic anchor and \texttt{poly-core v1} under a development protocol that uses validation for method selection and reports test only once for the final frozen comparison.

Table~\ref{tab:robustness_results} summarizes the revised picture. Under the random split, \texttt{poly-core v1} still shows a small positive delta over the tuned logistic anchor on the test partition. However, paired bootstrap analysis yields a 95\% confidence interval of \([-0.0115, 0.0260]\), so the random-split gain is not statistically decisive. Under the DOI-grouped split, the direction reverses: tuned logistic reaches 0.1250 macro-F1 on the grouped test partition, whereas \texttt{poly-core v1} reaches 0.1140. The corresponding bootstrap interval for the difference is \([-0.0254, 0.0020]\). In other words, the current feature augmentation is informative for ablation and representation analysis, but not yet robust enough to support a strong grouped-generalization claim.
...
\begin{tabular}{lcc}
\toprule
Test comparison & $\Delta$ Macro-F1 & 95\% CI \\
\midrule
Random split & +0.0068 & [-0.0115, 0.0260] \\
DOI-grouped split & -0.0110 & [-0.0254, 0.0020] \\
\bottomrule
\end{tabular}
\end{table}
...
The revised experiments support a narrower and more credible conclusion than the original project hypothesis. The current benchmark clearly supports a strong sparse anchor and a set of feature-level insights, but it does not yet support a strong claim that \texttt{poly-core v1} is robustly superior across stricter generalization settings. This distinction matters because the benchmark labels are weakly supervised and positive-unlabeled in character, and because DOI-grouped splitting substantially changes the empirical ranking.

From a bioinformatics perspective, this is still a useful result. It clarifies that benchmark construction, split design, and label semantics are at least as important as model class in this problem family. It also shows that the most useful structural signals under current public-data conditions are not the broadest or most elaborate ones, but a compact subset centered on molecular-weight, residue-set, and branching information.
...
The benchmark construction scripts, split definitions, baseline implementations, and supplementary benchmark statistics used in this study are available in the project repository and associated submission package. The main manuscript results are based on the DoLPHiN-derived filtered benchmark artifact \texttt{dataset\_publishable\_supervised\_v1} together with the fixed random and DOI-grouped split files described in the supplementary reproducibility materials. Replace this paragraph with the final repository URL and persistent identifiers at submission time.

\paragraph{Competing interests.}
The authors declare that they have no competing interests.

\paragraph{Funding.}
[Insert funding information here. If none: Not applicable.]

\paragraph{Authors' contributions.}
[AA] designed the study. [AA] and [BB] implemented the benchmark and experiments. [AA], [BB], and [CC] analyzed the results. [AA] drafted the manuscript. All authors revised the manuscript and approved the final version.

\paragraph{Acknowledgements.}
[Insert acknowledgements here. If none: Not applicable.]

\paragraph{Authors' information.}
[Optional author-information paragraph, or Not applicable.]

\paragraph{Limitation.}
Our benchmark is intentionally conservative and single-source in its main supervised setting. Although we implemented a merged \texttt{DoLPHiN + CSDB} pipeline, the currently ingested CSDB slice is not label-aligned enough to support a strong cross-source supervised claim. In addition, explicit evidence-level annotations are largely unavailable in the retained supervised data, which limits the strength of any current evidence-aware modeling result. More fundamentally, the benchmark labels are literature-derived positives with unknown negatives, so the task is closer to weakly supervised or positive-unlabeled multi-label prediction than to fully observed classification. The revised DOI-grouped split also shows that the current feature augmentation does not yet deliver robust grouped generalization, even though it remains informative in random-split ablations. Future work should therefore focus on better evidence annotation, stronger source alignment, more chemically faithful cross-database structural normalization, PU-aware evaluation and learning protocols, and grouped generalization tests rather than on scaling model size alone.

\paragraph{Reproducibility note.}
To reduce ambiguity between the benchmark claim and the engineering scaffold, we provide a supplementary reproducibility package that lists the exact benchmark artifact names, split statistics, tuned-anchor configuration, and final token-family definition used for \texttt{poly-core v1}.

\begin{figure}[t]
\centering
\includegraphics[width=0.98\linewidth]{../../figures/figure1_pipeline.pdf}
\caption{Overview of the proposed benchmark construction and \texttt{poly-core v1} pipeline.}
\label{fig:pipeline}
\end{figure}
