"""Run a linear SVM multilabel baseline over structured text features."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run linear SVM multilabel baseline")
    parser.add_argument("--dataset", type=Path, default=DATA_PROCESSED / "dataset_publishable_supervised_v1.jsonl")
    parser.add_argument(
        "--split",
        type=Path,
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=EXPERIMENTS / "revision_p1" / "linear_svm_random_test.json",
    )
    parser.add_argument("--eval-split", type=str, default="test", choices=["valid", "test"])
...
def main() -> None:
    ...
    for split_def in expanded_splits:
        ...
        y_pred_bin = ensure_2d_binary_predictions(classifier.predict(x_test), num_labels=len(mlb.classes_))
    ...
    result = {
        ...
        "note": "Linear SVM baseline over TF-IDF structured text features",
    }
    write_json(args.output, result)
    print(f"Wrote linear SVM baseline result to {args.output}")
